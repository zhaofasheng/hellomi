...
class FakeViewerUserListTileWidget extends StatefulWidget {
  const FakeViewerUserListTileWidget({
    super.key,
    required this.index,
    this.indexData,
  });
  ...
  @override
  State<FakeViewerUserListTileWidget> createState() => _ViewerUserListTileWidgetState();
}

class _ViewerUserListTileWidgetState extends State<FakeViewerUserListTileWidget> {
  final controller = Get.find<FakeLiveController>();
  ...
  void onClickFollow() async {
    if (widget.indexData?.userId != Database.loginUserId) {
      ...
      controller.fakeLiveModel?.liveViewers[widget.index].isFollow = isFollow.value;
    } else {
