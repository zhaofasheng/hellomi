...
import 'package:tingle/common/api/edit_video_api.dart';
...
class EditVideoController extends GetxController {
  ...
  Future<void> onUploadVideo() async {
    ...
    if (InternetConnection.isConnect.value) {
      try {
        editVideoModel = await EditVideoApi.callApi(
          token: token,
          uid: uid,
          videoId: videoId,
          caption: captionController.text.trim(),
          hashTagId: selectedHashtagIds.join(","),
          videoImage: videoThumbnail != "" ? videoThumbnail : null,
        );

        if (editVideoModel?.status == true) {
          Get.back();
          Utils.showToast(text: editVideoModel?.message ?? "");
        } else {
          Utils.showToast(text: editVideoModel?.message ?? "");
        }
      } catch (e) {
        Utils.showLog("Upload Failed => $e");
      }
    } else {
